	...
	for (unsigned int l=0; l<noIndiv; l++)
	{
		...
		for (unsigned int j=0; j<noLoci; j++)
		{
			...
			for (unsigned int m=0; m <= 1; m++)
			{
				...
				//char targ[]={l}, targ2[]={j};
				//04-June-2018: "fixed" by casting as char.  This is still a terrible fix because it is converting a 32-bit int to an 8-bit char
				char targ[]={(char)l}, targ2[]={(char)j};
